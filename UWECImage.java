...
public class UWECImage {
	...
	public BufferedImage rotateImage(UWECImage im) {
		BufferedImage bi = new BufferedImage(this.getHeight(), this.getWidth(), BufferedImage.TYPE_INT_RGB);
		for(int i = 1; i<this.getWidth(); ++i) {
			for(int j = 1; j<this.getHeight(); ++j) {
				bi.setRGB(j, i, this.getImage().getRGB(i, j));
			}
		}
		return bi;
	}
	...
	public void switchImage(BufferedImage theNewImage) {
		this.im = theNewImage;
	}
}